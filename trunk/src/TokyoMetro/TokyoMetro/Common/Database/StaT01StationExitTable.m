...
-(void)item:(NSString *)item value:(id)value{
    if ([item isEqualToString:STAT01_STAT_EXIT_ID]) {
        self.statExitId = value;
    }else if ([item isEqualToString:STAT01_LINE_ID]){
        self.lineId = value;
    }else if ([item isEqualToString:STAT01_STAT_ID]){
        self.statId = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_NUM]){
        self.statExitNum = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_NAME]){
        self.statExitName = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_TYPE]){
        self.statExitType = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_LON]){
        self.statExitLon = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_LAT]){
        self.statExitLat = value;
    }else if ([item isEqualToString:STAT01_STAT_EXIT_FLOOR]){
