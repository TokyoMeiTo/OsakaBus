...
class Cmn003Dao:MstT02StationTable {
    ...
    let QUERY_MINI_DISTANCE = "select distinct STAT_GROUP_ID from MSTT02_STATION where LINE_ID LIKE '280%%' order by (STAT_LON - ?) * (STAT_LON - ?) + (STAT_LAT - ?)*(STAT_LAT - ?) limit 0,10"
