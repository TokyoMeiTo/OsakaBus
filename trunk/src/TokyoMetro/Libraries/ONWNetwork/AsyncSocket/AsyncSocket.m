...
- (BOOL)configureSocketAndReturnError:(NSError **)errPtr
{
	...
	if(theDelegate && [theDelegate respondsToSelector:@selector(onSocketWillConnect:)])
	{
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		if (theDelegate && [theDelegate respondsToSelector:@selector(onSocket:didAcceptNewSocket:)])
			[theDelegate onSocket:self didAcceptNewSocket:newSocket];
		
		newSocket->theFlags |= kDidStartDelegate;
		
		NSRunLoop *runLoop = nil;
		if (theDelegate && [theDelegate respondsToSelector:@selector(onSocket:wantsRunLoopForNewSocket:)])
		{
...
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		if (theDelegate && [theDelegate respondsToSelector:@selector(onSocket:didConnectToHost:port:)])
		{
...
- (void)closeWithError:(NSError *)err
{
	...
	if (theFlags & kDidStartDelegate)
	{
		...
		if (theDelegate && [theDelegate respondsToSelector:@selector(onSocket:willDisconnectWithError:)])
		{
...
- (void)close
{
	...
	{
		if (theDelegate && [theDelegate respondsToSelector: @selector(onSocketDidDisconnect:)])
		{
...
- (void)doBytesAvailable
{
	...
	else if (totalBytesRead > 0)
	{
		...
		if (theDelegate && [theDelegate respondsToSelector:@selector(onSocket:didReadPartialDataOfLength:tag:)])
		{
...
- (void)completeCurrentRead
{
	...
	if(theDelegate && [theDelegate respondsToSelector:@selector(onSocket:didReadData:withTag:)])
	{
